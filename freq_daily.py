...
# 附录评论（音乐/影视/魔卡）整体排除；附录4草稿纸属于日常保留
...
FOREIGN_KEYWORDS = ["Orchestra", "Orchestre", "partners", "Sonata", "Fantasia", "Symphony", "Season One", "Ep.", "Op."]

def keep_line(line):
    s = line.strip()
    if not s:
        return False
    if s.startswith("#"):  # 标题保留
        return True
    ...
    if BLOCK_START.match(s):
        return False
    if MUSIC_LINE.search(s):
        return False
    if STRONG_SIGNAL.search(s):
        return False
    if KTV_SONG.match(s):
        return False
    if CARD_DEF.search(s):
        return False
...
def count_year(num, label):
    text_parts = []
    for c in data[str(num)]:
        if c["id"] in SKIP_IDS:
            continue
        body = extract_comment(c["body"])
        body = re.sub(r"\[\^[^\]]+\]", "", body)          # 残留脚注引用
...
c25, n25 = count_year(27, "2025")
c26, n26 = count_year(28, "2026")

def show(title, c, n, k=50):
    print(f"\n########## {title} (字数 {n}) ##########")
    for w, cnt in c.most_common(k):
        print(f"{cnt}\t{w}")

show("2025 日常高频词", c25, n25)
show("2026 日常高频词", c26, n26)

s25 = {w for w, x in c25.items() if x >= 3}
s26 = {w for w, x in c26.items() if x >= 3}
print("\n########## 2025 日常特有(>=3次) TOP25 ##########")
for w, cnt in sorted([(w, c25[w]) for w in s25 - s26], key=lambda x: -x[1])[:25]:
    print(f"{cnt}\t{w}")
print("\n########## 2026 日常特有(>=3次) TOP25 ##########")
for w, cnt in sorted([(w, c26[w]) for w in s26 - s25], key=lambda x: -x[1])[:25]:
    print(f"{cnt}\t{w}")

print("\n########## 生活/兴趣词汇检索 ##########")
for key in ["酸奶", "可乐", "奶茶", "蛋糕", "咖啡", "蚂蚁", "种树", "电影", "专辑", "周杰伦",
            "KTV", "唱歌", "跑步", "健身", "加班", "甜品", "饮料", "电费", "水费", "燃气",
            "地铁", "公交", "高铁", "火车", "机票", "出差", "旅游", "医院", "生病", "感冒",
            "数学", "物理", "公式", "代码", "游戏", "博德", "麻将", "扑克", "读书", "看书",
            "抄书", "书法", "画画", "草稿纸", "魔方", "篮球", "游泳", "散步", "爬", "走路",
            "新年", "春节", "中秋", "端午", "清明", "元旦", "生日", "考试", "世界杯", "华为",
            "手机", "相机", "平板", "电脑", "键盘", "耳机", "苹果", "小米", "耳机", "平板"]:
    v25 = c25.get(key, 0); v26 = c26.get(key, 0)
    if v25 or v26:
        print(f"{key}: 2025={v25}  2026={v26}")